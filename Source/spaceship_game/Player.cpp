...
constexpr float MISSILE_DELAY = 0.0f;  // Time between firing missiles
constexpr float LASER_DELAY = 0.2f;    // Time between firing lasers
constexpr int   NUM_PARTICLES = 350;   // How many particles are spawned when the ship explodes
constexpr float BLINK_INTERVAL = 0.2f; // Duration between blinks while invincible

constexpr int   WIDTH = 36;
constexpr int   HEIGHT = 40;

Player::Player(float x, float y, int player_num, int screen_w, int screen_h, EntityList& entities, string name) :
    Entity(x, y, WIDTH, HEIGHT, screen_w, screen_h),
    _entities{entities},
    _vx{},
	_vy{},
    _turnSpeed{3.0f},
	_linearAcceleration{300.0f},
    _maxSpeed{300.0f},
...
void Player::shootMissile()
{
    if (_missileCooldown == 0)
    {
        ...
        _entities.push_back(new Missile(_x, _y, speed, _angle, _playerNum, _screenW, _screenH, _entities));
		_event.invoke(EVENT::MISSILE_SHOT, _playerName);
    }
}
...
void Player::shootLaser()
{
    if (_laserCooldown == 0)
    {
        ...
        _entities.push_back(new Laser(_x, _y, speed, _angle, _playerNum, _screenW, _screenH, _entities));
        _event.invoke(EVENT::LASER_SHOT, _playerName);
    }
}
    ...
    if (_timeAlive >= _invincibleTime || _hasFired)
    {
        ...
        case 1:
            ...
            if (dynamic_cast<Missile*>(&entity)->getPlayerNum() != _playerNum)
            {
                ...
                _alive = false;
				_event.invoke(EVENT::SHIP_DESTROYED, _playerName);
            }
            break;
        case 4:
            // If colliding with a Laser, die when it's from another Player
