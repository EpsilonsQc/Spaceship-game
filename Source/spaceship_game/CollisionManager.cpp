...
{
    float multiplier = (point.x * axis.x + point.y * axis.y) / (powf(axis.x, 2) + powf(axis.y, 2));

    return multiplier * powf(axis.x, 2) + multiplier * powf(axis.y, 2);
}

// Determine minimum and maximum scalar values from all corners on a given axis
...
// Return true if two given hit boxes are overlapping as projected on a given axis
